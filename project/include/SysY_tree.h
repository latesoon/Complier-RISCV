...
//add 2024.11.22
std::vector<Expression>* get_params(){return new std::vector<Expression>{};};
...
public:
    ...
    //add 2024.11.22
    std::vector<Expression>* get_params(){return params;}
};
...
public:
    ...
    virtual std::vector<InitVal> * get_initval()=0;
};
...
public:
    ...
    std::vector<InitVal> * get_initval(){return initval;}
};
...
public:
    ...
    std::vector<InitVal> * get_initval(){return NULL;}
};
...
public:
    ...
    //add 2024.11.19 2024.11.20 2024.11.22
    std::vector<int> dimnum{};
    virtual Symbol get_name() const = 0;
    virtual void set_type(Type::ty t)=0;
    virtual InitVal get_init() const =0;
    virtual std::vector<Expression> * get_dims() const=0;
    virtual bool constcheck()=0;
};
...
public:
    ...
    //add 2024.11.22
    Type::ty type;
    void set_type(Type::ty t){type=t;}

    // 如果dims为nullptr, 表示该变量不含数组下标, 你也可以通过其他方式判断，但需要修改SysY_parser.y已有的代码
    VarDef_no_init(Symbol n, std::vector<Expression> *d) : name(n), dims(d) {}
    
    //add 2024.11.20 2024.11.22
    Symbol get_name() const { return name; }
    InitVal get_init() const {return InitVal();} 
    std::vector<Expression> * get_dims() const {return dims;}
    bool constcheck();
...
public:
    ...
    //add 2024.11.20 2024.11.22
    Symbol get_name() const { return name; }
    InitVal get_init() const {return init;} 
    std::vector<Expression> * get_dims() const {return dims;}
    bool constcheck();
...
public:
    ...
    //add 2024.11.20
	virtual std::vector<Def>* get_var_def_list() const = 0;
	virtual Type::ty get_type_decl() const=0;
};
...
public:
    ...
    //add 2024.11.20
    std::vector<Def>* get_var_def_list() const {return var_def_list;}
    Type::ty get_type_decl() const{return type_decl;}
